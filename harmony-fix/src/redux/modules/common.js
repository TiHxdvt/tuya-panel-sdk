...
// 延迟获取 putDpData，避免模块加载时 TYSdk.device 为 undefined 导致崩溃
const putDpData = (...args) => TYSdk.device.putDpData(...args);
...
const dpUpdateEpic$ = action$ =>
  action$.ofType(updateDp).mergeMap(action => {
    ...
    const [success, error] = Observable.fromPromise(putDpData(payload))
